...
import { getAllCategories, getAllLocals, getCardById, getCards, getCepForCard, handleCreateCard, handleEmailLogin, handleUpdateCard } from '@/services/backendCalls';
...
import { BoxContainerStyle, openMapButtonStyle } from "../createCard/stylesMui";
    ...
    useEffect(()=> {
        const fetchData = async () => {
            ...
            if(cardData){
                ...
                setLatLngFromCep([Number(cardData.lat_cupom), Number(cardData.lon_cupom)])
                setImageUrl(cardData.img_url);
                setcategorySelected(cardData.id_category);
                setlocalSelected(cardData.id_local);

                const foundLocal = localsResponse.find(local => local.ibeacons_id_locals === cardData.id_local);
                if (foundLocal) {
                    setLocalSelectedName(foundLocal.ibeacon_local);
                }
                const foundCategory = categoryResponse.find(category => category.id_cat === cardData.id_category);
                if(foundCategory){
